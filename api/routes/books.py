...
async def create_books(book: BookBase, db: db_dependency):
    db_book = models.Book(api_id=book.api_id, title=book.title, authors=book.authors, cover_url=book.cover_url, genres=book.genres, description=book.description, published_year=book.published_year)
    db.add(db_book)
    db.commit()
    return db_book
